...
export default async function Station() {
  ...
  const kid = kids[0];
  const lifetime = kids.reduce((s, k) => s + k.lifetime_points, 0);
  const st = stationFor(lifetime);
  return (
    <main className="safe-x safe-bottom mx-auto max-w-3xl">
      <TopBar back={`/k/${kid?.id ?? "blake"}`} title="Space station" />
      <Panel className="flex flex-col items-center">
        ...
        <div className="numeral mt-3 text-4xl text-[#7dd3fc]">{fmtPts(lifetime)} pts</div>
        <div className="text-ink-2 text-sm font-bold">every point you have ever earned builds the station</div>
        {st.next && (
          <div className="mt-3 w-full">
            ...
            <div className="text-ink-2 mt-1 text-center text-sm font-bold">
              {fmtPts(st.next.min - lifetime)} more points builds the {st.next.name}
            </div>
          </div>
        )}
      </Panel>
      <h2 className="mt-4 px-1 text-lg font-extrabold">Every level unlocks a new part in the shop</h2>
